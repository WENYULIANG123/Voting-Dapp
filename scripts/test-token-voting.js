...
async function main() {
  console.log("=== Token Voting System Test ===");
  ...
  const registrationStart = currentTime + 10; // 10 seconds from now
  const registrationEnd = currentTime + 40; // 40 seconds from now
  const votingStart = currentTime + 50; // 10 seconds after registration ends
  const votingEnd = currentTime + 80; // 80 seconds from now
  ...
  const accounts = [voter1, voter2, voter3, candidate1, candidate2];
  ...
  try {
    ...
    if (candidates.length >= 2) {
      ...
      const voteAmount3 = ethers.parseEther("150"); // Vote with 150 tokens
      ...
      await simpleToken.connect(voter3).approve(await tokenVoting.getAddress(), voteAmount3);
      ...
      const voteTx3 = await tokenVoting.connect(voter3).vote(1, candidates[1], voteAmount3);
      await voteTx3.wait();
      console.log("Voter 3 voted with 150 tokens for Bob Smith");
    }
  } catch (error) {
